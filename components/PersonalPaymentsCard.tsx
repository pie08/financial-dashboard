 ...
 */
export default function PersonalPaymentsCard({ comparison }: { comparison: PeriodComparison }) {
  const { current, previous } = comparison;
  const pct = previous ? pctChange(current.personalIn, previous.personalIn) : null;

  return (
    <div className="flex flex-wrap items-center gap-x-6 gap-y-2 rounded-xl border border-hairline bg-surface p-4">
      <div className="min-w-0">
        <p className="text-xs text-muted">Personal payments · {current.label}</p>
        <p className="mt-1 text-xl font-semibold text-ink sm:text-2xl">{money(current.personalIn)}</p>
        {pct !== null ? (
          ...
          <p className="mt-1 text-xs text-muted">no transfers in {previous.label}</p>
        ) : null}
      </div>
      <div className="ml-auto max-w-xs text-right">
        {current.personalOut > 0 && (
          <p className="text-xs text-ink-2">
            {money(current.personalOut)} <span className="text-muted">moved back to business</span>
          </p>
        )}
